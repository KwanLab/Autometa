...
#0. Standardize fasta contig names
for index,row in ref_genome_table.iterrows():
    basename = row['basename']
    genome_name = row['ref_genome']
    genome_filename_dict[basename] = genome_name

for fna in glob.glob("*.fna"):
    basename = ".".join(fna.split(".")[:-1])
    output_handle = basename + ".fasta"
    ...
    with open(aln) as infile:
        ...
        for line in infile:
            ...
            if ">" in line:
                ...
                if m:
                    genome_basename = m.group()[1:-2].rstrip("_genomic")
