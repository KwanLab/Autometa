...
def restricted_float(x):
    ...
    if x < 0.0 or x > 1.0:
        raise argparse.ArgumentTypeError(
            "%r is not in the brightness range. Brightness must be between 0 and 1. "
            % (x,)
        )
    return x
...
def contig_selection(source=None, source2=None):
    return CustomJS(
        args=dict(source=source, source2=source2),
        code="""
    var bk_obj = cb_obj;
...
parser = argparse.ArgumentParser(
    description="Builds a scatter plot of clusters from AutoMeta output",
    epilog="Output is generated as an html file. Open the file in your browser to view the clustering.",
)
parser.add_argument(
    "-i",
    metavar="<autometa_table>",
    help="Table generated from AutoMeta output (ML_recruitment_output.tab)",
    required=True,
)
parser.add_argument(
    "-o",
    metavar="<outfile>",
    help="prefix to the generated html file",
    default="autometa_clustering",
    required=False,
)
parser.add_argument(
    "-b",
    metavar="<brightness>",
    help="Adjusts the color scheme hue.",
    default=0.70,
...
parser.add_argument(
    "-legend",
    action="store_true",
    help="Adds legend to the scatter plot",
    required=False,
)
parser.add_argument(
    "-plot_title",
    metavar="<plot_title>",
    help="text to place as the title of the scatter plot",
    default="AutoMeta Clustered Contigs",
    required=False,
)
...
crcolor, crRGBs = generate_color_range(
    len(df.cluster.unique()), brightness
)  # produce spectrum
...
gc_plot.segment(
    ...
    line_color=factor_cmap(
        "cluster", factors=list(df.cluster.unique()), palette=crcolor
    ),
    source=source,
    name="gc%",
)
