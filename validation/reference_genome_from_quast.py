...
parser = argparse.ArgumentParser(
    description="Script to assign reference genome\
    to de novo contig using nucmer alignment produced by quast"
)
parser.add_argument(
    "-a", "--assembly", help="De novo assembly nucl fasta file", required=True
)
parser.add_argument(
    "-c", "--coordinate_table", help="Nucmer *.coords* table", required=True
)
parser.add_argument(
    "-f", "--filtered", help="Nucmer .coords table is filtered", default=True
)
parser.add_argument(
    "-o", "--out_table", help="Output.tsv", default="contig_reference.tsv"
)
...
def parse_filtered_nucmer_table(coords_table, filtered=True):
    # 1. Load table into pandas df
    filtered_coordinate_table = pd.read_csv(
        coords_table, header=None, skiprows=2, sep="|"
    )
    # 2. Populate dictionary, key = contig, value = reference genome
    alignment_dict = {}
...
print(
    (
        "{} of {} contigs were unaligned by nucmer...".format(
            len(unaligned_contigs), len(contig_list)
        )
    )
)
