...
def is_alignment_congruent_with_ref(
    read_name, contig_aligned_to, read_ranges, contig_species
):
...
parser = argparse.ArgumentParser(
    description="Script to assess the metagenome deconvolution method"
)
parser.add_argument(
    "-r", "--refsam", help="sam alignment to ref genomes", required=True
)
parser.add_argument(
    "-s", "--refspecies", help="ref contig species table", required=True
)
...
parser.add_argument(
    "-b",
    "--bintable",
    help="bin classification table for assembly",
    nargs="+",
    required=True,
)
...
number_sam_lines = (
    int(last_read) * 2
) + number_of_lines  # We assume paired-end reads here
...
non_unique_reads = (
    {}
)  # Dictionary that just contains reads found in more than one genome
...
print(
    ...
    + " Working out how many unique reads there are per genome..."
)
...
print(
    strftime("%Y-%m-%d %H:%M:%S") + " Parsing assembly SAM, counting species reads..."
)
contig_classifications = (
    {}
)  # Dictionary of dictionaries, which will hold running tallies of reads assigned to different species
    ...
    with gzip.open(asm_sam_path, "rb") as asm_sam:
        for line in tqdm(asm_sam, total=number_sam_lines):
            ...
            if not line[0] == "@":
                ...
                read_name = line_list[0]
                contig_name = line_list[2]
                read_species = species_classification_for_read(
                    read_name, ranges, non_unique_reads
                )  # Returns None if a non-unique read
                if read_species:
...
for contig in tqdm(contig_classifications, total=number_of_contigs):
    ...
    for species in contig_classifications[contig]:
        chimera_table.write(
            ...
            + species
            + "\t"
            + str(contig_classifications[contig][species])
            + "\t"
            + str(percents[species])
            + "\n"
        )
chimera_table.close

# 5a. Make a table that determines the yield of genome reads that align to contigs (i.e. judge the quality of the assembly)
aligned_unique_reads = (
    {}
)  # Dictionary that will store totals of aligned reads for each species
reads_in_pure_contigs = (
    {}
)  # Dictionary that will store totals of aligned reads in 'pure' > 90% contigs
reads_in_chimeric_contigs = (
    {}
)  # Dictionary that will store totals of aligned reads in 'impure' < 90% contigs
...
print(
    strftime("%Y-%m-%d %H:%M:%S")
    + " Writing assembly recovery table "
    + asm_recovery_table_path
    + "..."
)
for contig in contig_classifications:
    ...
    for species in percents:
        ...
        if percents[species] >= 90:
            ...
            reads_in_chimeric_contigs[species] += contig_classifications[contig][
                species
            ]

# Write out table
asm_recovery_table = open(asm_recovery_table_path, "w")
asm_recovery_table.write(
    "species\taligned_unique_reads\treads_in_pure_contigs\treads_in_chimeric_contigs\n"
)
for species in aligned_unique_reads:
    asm_recovery_table.write(
        species
        + "\t"
        + str(aligned_unique_reads[species])
        + "\t"
        + str(reads_in_pure_contigs[species])
        + "\t"
        + str(reads_in_chimeric_contigs[species])
        + "\n"
    )
...
for bin_classifications_table_path in bin_classifications_table_paths:
    ...
    if not bin_column_index:
        print(
            ...
            + bin_column
            + " in bin table "
            + bin_classifications_table_path
        )
        sys.exit(2)
    ...
    total_contigs = len(list(contig_bins.keys()))
    ...
    print(
        strftime("%Y-%m-%d %H:%M:%S")
        + " Writing binning accuracy table "
        + bin_accuracy_table_path
        + "..."
    )
    ...
    for bin_name in bin_classifications:
        for species in bin_classifications[bin_name]:
            ...
            if bin_name in genome_reads_in_bins[species]:
                genome_reads_in_bins[species][bin_name] += bin_classifications[
                    bin_name
                ][species]
            else:
                genome_reads_in_bins[species][bin_name] = bin_classifications[bin_name][
                    species
                ]
    ...
    print(
        strftime("%Y-%m-%d %H:%M:%S")
        + " Writing binning recovery table "
        + bin_recovery_table_path
        + "..."
    )
    bin_recovery_table = open(bin_recovery_table_path, "w")
    bin_recovery_table.write(
        "genome\tbin\treads\tpercent_of_input\tpercent_of_aligned_to_asm\n"
    )
    ...
    for species in genome_reads_in_bins:
        ...
        percents_unique_input = (
            {}
        )  # Percent expressed as a fraction of the unique reads aligned to reference genomes
        percents_aligned_to_contigs = (
            {}
        )  # Percent expressed as a fraction of the unique reads aligned to the assembly
        for bin_name in genome_reads_in_bins[species]:
            number_of_reads = genome_reads_in_bins[species][bin_name]
            percent_unique_input = (
                float(number_of_reads) / float(number_of_unique_reads[species])
            ) * 50  # Here we assume paired reads
            percent_aligned_to_contigs = (
                float(number_of_reads) / float(aligned_unique_reads[species])
            ) * 50
            percents_unique_input[bin_name] = percent_unique_input
            percents_aligned_to_contigs[bin_name] = percent_aligned_to_contigs

        recovery_values_list = list(percents_aligned_to_contigs.values())
        ...
        sorted_bins = sorted(
            percents_aligned_to_contigs,
            key=percents_aligned_to_contigs.get,
            reverse=True,
        )
        ...
        for bin_name in genome_reads_in_bins[species]:
            bin_recovery_table.write(
                ...
                + bin_name
                + "\t"
                + str(genome_reads_in_bins[species][bin_name])
                + "\t"
                + str(percents_unique_input[bin_name])
                + "\t"
                + str(percents_aligned_to_contigs[bin_name])
                + "\n"
            )
    ...
    while len(genome_recovery_values) < len(list(ranges.keys())):
        genome_recovery_values.append(float(0))

    bin_accuracy_average = float(sum(bin_accuracy_values)) / len(bin_accuracy_values)
    genome_recovery_average = float(sum(genome_recovery_values)) / len(
        genome_recovery_values
    )
...
for table in clustering_quotients:
    clustering_table.write(
        table
        + "\t"
        + str(bin_accuracy_averages[table])
        + "\t"
        + str(genome_recovery_averages[table])
        + "\t"
        + str(total_bin_genome_recovery[table])
        + "\n"
    )
