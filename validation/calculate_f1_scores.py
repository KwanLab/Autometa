...
parser = argparse.ArgumentParser(
    description="Script to Calculate F1 from master\
 contig table (for heat map)."
)
parser.add_argument(
    "-c", "--cluster_column", help="Name of the cluster column", required=True
)
parser.add_argument(
...
parser.add_argument(
    "-u",
    "--unclustered_term",
    help='Name of unclustered bin. Default = "unclustered"',
    default="unclustered",
)
parser.add_argument(
    "-o", "--out", help="Tab delimited output table with F1 scores", default="F1.tab"
)
...
contig_table[args["cluster_column"]].astype(str)
...
def find_dominant_cluster_length(contig_list, contig_dict, reference_genome):
    ...
    try:
        assert list(cluster_dict.values()).count(max_cluster_length) <= 1
    except AssertionError:
        num_clusters_with_same_mapped_length = list(cluster_dict.values()).count(
            max_cluster_length
        )
        print()
        ...
        print(
            (
                "Warning: There are {} clusters that share a max aligned length of {} for {}".format(
                    num_clusters_with_same_mapped_length,
                    max_cluster_length,
                    reference_genome,
                )
            )
        ...
        dominant_clusters = sorted(
            list(cluster_dict.items()), key=operator.itemgetter(1)
        )[-num_clusters_with_same_mapped_length:]
        # print cluster_dict
    ...
    dominant_cluster = str(
        max(iter(cluster_dict.items()), key=operator.itemgetter(1))[0]
    )
    # if the "dominant" cluster is unclustered, then use next in line, if available
    if (
        dominant_cluster == args["unclustered_term"]
        and len(list(cluster_dict.keys())) > 1
    ):
        ...
        print(
            (
                "Dominant cluster in '{}', using second largest cluster ({}) instead.".format(
                    args["unclustered_term"], dominant_cluster
                )
            )
        )
...
with open(args["out"], "w") as outfile:
    outfile.write(
        "ref_genome\tref_genome_len\tdominant_cluster\ttotal_dominant_cluster_len\tref_aln_dominant_cluster_len\trecall\tprecision\tF1\n"
    )
    print(
        "ref_genome\tref_genome_len\tdominant_cluster\ttotal_dominant_cluster_len\tref_aln_dominant_cluster_len\trecall\tprecision\tF1\n"
    )
    for reference_genome, contig_list in list(reference_genome_dict.items()):
        dominant_cluster, dominant_cluster_length = find_dominant_cluster_length(
            contig_list, contig_dict, reference_genome
        )
        reference_genome_length = sum(
            [contig_dict[contig]["length"] for contig in contig_list]
        )
        if dominant_cluster == args["unclustered_term"]:
            ...
            recall = round(
                dominant_cluster_length / float(reference_genome_length) * 100, 5
            )
