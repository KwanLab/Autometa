...
if taxonomy_table_path:
    ...
    if os.path.isfile(taxonomy_table_path):
        if output_dir_absolute != taxonomy_directory:
            run_command(
                "cp " + taxonomy_table_path_absolute + " " + output_dir_absolute + "/"
            )
...
if taxonomy_table_path:
    autometa_command = autometa_command + " --taxonomy_table /output/{}".format(
        taxonomy_table_filename
    )
...
if cov_table:
    ...
    autometa_command = autometa_command + " --cov_table /output/{}".format(
        cov_table_filename
    )
