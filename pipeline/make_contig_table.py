...
parser = argparse.ArgumentParser(
    description="Script to make a gc, length and coverage table from an assembly",
...
parser.add_argument(
    ...
    help="Path to coverage table made by calculate_read_coverage.py \
...
parser.add_argument(
    ...
    "--output",
    metavar="<output.tab>",
    help="Path to create output table",
    required=True,
)
parser.add_argument(
    "-n",
    "--no_coverage",
    help="Specifies not to try to add coverage to the table",
    action="store_true",
)
...
if coverage_table_path is not None:
    ...
    if os.path.isfile(coverage_table_path):
        ...
        print(
            ("Error, couldnt find file " + coverage_table_path + " or it is unreadable")
        )
        quit()
...
for seq_record in SeqIO.parse(fasta_assembly_path, "fasta"):
    ...
    if use_coverage_table:
        ...
        if (
            contigList[0] == "NODE"
            and contigList[2] == "length"
            and contigList[4] == "cov"
        ):
            ...
            print(
                (
                    "Error, "
                    + contig
                    + " not the right format to extract coverage from sequence name"
                )
            )
            quit()
        output.write(contig + "\t" + length + "\t" + gc + "\t" + str(cov) + "\n")
