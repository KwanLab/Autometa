...
def restricted_float(x):
    ...
    if x < 0.0 or x > 1.0:
        raise argparse.ArgumentTypeError(
            "%r not in range between 0 and 1. Must convert the percentage to a decimal"
            % (x,)
        )
    return x
...
parser = argparse.ArgumentParser(
    description="Script to find the Lowest Common Ancestor for each ORF from a BLAST output table",
...
subparsers = parser.add_subparsers(
    title="Method for Accession of Database Files",
    help="Additional help may be provided by specifying (database_files|database_directory) -h",
)
parser_databasefiles = subparsers.add_parser(
    "database_files",
...
parser_databasefiles.add_argument(
    "accession2taxid", help="Path to prot.accession2taxid file"
)
parser_databasefiles.set_defaults(
    parser_databasedirectory=False, parser_databasefiles=True
)

parser_databasedirectory = subparsers.add_parser(
    "database_directory", help="Accesses database files from directory provided"
)
parser_databasedirectory.add_argument(
...
parser_databasedirectory.set_defaults(
    parser_databasefiles=False, parser_databasedirectory=True
)

parser.add_argument("blast", metavar="BLAST output", help="Path to BLAST output file.")
parser.add_argument(
    "-f",
    metavar="bitscore filter",
    required=False,
    default=0.9,
...
accession2taxid_dict = lca_functions.Process_accession2taxid_file(
    accession2taxid_file, blast_orfs
)
...
for orf, taxset in blast_taxids.items():
    lca = False
    while lca == False:
        ...
        if len(taxset) >= 2:
            try:
                ...
                lca = reduce(
                    lambda taxid1, taxid2: lca_functions.RangeMinQuery(
                        node1=taxid1,
                        node2=taxid2,
                        tree=tour,
    ...
    for line in names_dmp:
        ...
        for i, value in enumerate(line_list):
            ...
            if line_list[3] == "scientific name":
                nospace_name = line_list[1].replace(
                    " ", "_"
                )  # This helps with parsing in R later
...
for orf in lca_dict.keys():
    ...
    if lca_dict[orf]["lca"] in reference_taxids.keys():
