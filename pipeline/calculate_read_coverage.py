...
def run_bowtie2(
    ...
    if not (len(paths_to_F_reads) == len(paths_to_R_reads)):
        print(
            "Error, you need to specify the same number of forward and reverse read files"
        )
        quit()
    ...
    bowtie2_command_string = (
        "bowtie2 -x "
        + bowtie2_db
        + " -q --phred33 --very-sensitive --no-unal -p "
        + str(num_processors)
        + " -S "
        + sam_file_name
    )
...
run_command(
    "samtools view -bS {} | samtools sort -o {}".format(sam_file, sorted_bam_file)
)
...
run_command(
    "genomeCoverageBed -ibam {} -g {}".format(sorted_bam_file, contig_length_tab_file),
    genome_bed_file,
)
