...
from os.path import isfile,isdir,abspath,basename,splitext
from os import stat,listdir,getcwd,chdir,makedirs
...
def update_dbs(database_path, db='all'):
	...
	if db == 'all' or db == 'nr':
		...
		if isfile(database_path + '/nr.gz.md5'):
	...
	if db == 'all' or db == 'acc2taxid':
		...
		if isfile(database_path + '/prot.accession2taxid.gz.md5'):
		...
		if isfile(database_path + '/prot.accession2taxid.gz'):
	...
	if db == 'all' or db == 'taxdump':
		...
		if isfile(database_path + '/taxdump.tar.gz.md5'):
		...
		if isfile(database_path + '/taxdump.tar.gz'):
...
def length_trim(fasta_path,length_cutoff):
	input_fname, ext = splitext(basename(fasta_path))
	#Trim the length of fasta file
	outfile_path = output_dir + '/' + input_fname + "_filtered.fasta"
...
def run_prodigal(path_to_assembly):
	assembly_filename = abspath(path_to_assembly).split('/')[-1]
...
def run_diamond(prodigal_output, diamond_db_path, num_processors, prodigal_daa):
	...
	if not isdir(tmp_dir_path):
		makedirs(tmp_dir_path) # This will give an error if the path exists but is a file instead of a dir
...
def run_blast2lca(input_file, taxdump_path):
	output = output_dir + '/' + '.'.join(abspath(input_file).split('/')[-1].split('.')[:-1]) + ".lca"
	if isfile(output) and not stat(prodigal_output + ".lca").st_size == 0:
...
def run_taxonomy(pipeline_path, assembly_path, tax_table_path, db_dir_path,
	...
	if bgcs_path:
		...
		unmasked_fname = basename(tax_table_path).replace('.lca', '.unmasked.tsv')
...
db_dir_path = abspath(args['db_dir'])
...
fasta_fname, _ = splitext(basename(fasta_path))
prodigal_output = output_dir + '/' + fasta_fname + "_filtered.orfs"
prodigal_daa = prodigal_output + ".daa"

# If cov_table defined, we need to check the file exists
if cov_table:
	if not isfile(cov_table):
...
if not isfile(pipeline_path+"/lca_functions.so"):
	cythonize_lca_functions()

if not isdir(db_dir_path):
...
elif not listdir(db_dir_path):
	#The 'Autometa databases' directory is empty
	print('AutoMeta databases directory empty, populating with appropriate databases.\n\
	This may take some time...')
	update_dbs(db_dir_path)

nr_files = ['nr.dmnd', 'nr.dmnd.md5', 'nr.gz.md5']
taxdmp_files = ['names.dmp', 'nodes.dmp', 'taxdump.tar.gz.md5']
acc2taxid_files = ['prot.accession2taxid.gz.md5', 'prot.accession2taxid']
db_files = listdir(db_dir_path)

for fh in nr_files:
	if fh not in db_files:
		print('NR database not found, downloading/formatting NR for DIAMOND.\n\
		This may take some time...')
		update_dbs(db_dir_path, 'nr')

for fh in acc2taxid_files:
	if fh not in db_files:
		print('acc2taxid files not found, downloading.\nThis may take some time...')
		update_dbs(db_dir_path, 'acc2taxid')

for fh in taxdmp_files:
	if fh not in db_files:
...
filtered_assembly = output_dir + '/' + fasta_fname + "_filtered.fasta"
if not isfile(filtered_assembly):
	filtered_assembly = length_trim(fasta_path, length_cutoff)

if not isfile(prodigal_output + ".faa"):
...
if not isfile(taxonomy_table) or stat(taxonomy_table).st_size == 0:
...
taxonomy_pd = read_table(taxonomy_table)
categorized_seq_objects = {}
all_seq_records = {}

# Load fasta file
for seq_record in parse(filtered_assembly, 'fasta'):
...
if not single_genome_mode:
	for kingdom in categorized_seq_objects:
		...
		write(seq_list, output_path, 'fasta')
