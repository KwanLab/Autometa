...
def runDBSCANs(
    table, dimensions, hmm_dictionary, domain, completeness_cutoff, purity_cutoff
):
    ...
    while number_of_clusters > 1:
        ...
        if completenessList:
            ...
            if (
                some_clusters_found
            ):  # I.e. at some point clusters were found, but not this time
                break
            else:
                number_rounds_with_zero_clusters += 1

        if (
            number_rounds_with_zero_clusters >= 10
    ...
    for cluster in complete_and_pure_clusters:
        subset_other_db_table = subset_other_db_table[
            subset_other_db_table["db_cluster"] != cluster
        ]
...
parser = argparse.ArgumentParser(
    description="Perform initial clustering via BH-tSNE and DBSCAN."
)
parser.add_argument(
    "-t",
    "--input_table",
    help="Master contig table. Optionally can contain taxonomy data",
    required=True,
)
...
if os.path.isfile(matrix_file):
    ...
    for contig_name in assembly_seqs:
        ...
        list_size = len(list(unique_k_mers.keys()))
...
if os.path.isfile(BH_tSNE_output_file):
    ...
    # Write file to disk
    master_table.to_csv(
        path_or_buf=BH_tSNE_output_file, sep="\t", index=False, quoting=csv.QUOTE_NONE
    )
...
for i, row in master_table.iterrows():
    ...
    if pfamString == "NA" or (
        isinstance(pfamString, numbers.Number) and math.isnan(pfamString)
    ):
        continue
...
if has_taxonomy_info and data_size > 50:
    for dimensions in [2, 3]:
        ...
        for taxonomic_level in taxonomic_levels:
            ...
            for classification in list(classification_dict.keys()):
                logger.info("Examining " + classification)
                # Get subset table
                subset_table = local_current_table.loc[
                    local_current_table[taxonomic_level] == classification
                ]  # COPY of local_current_table
                ...
                while True:
                    ...
                    (
                        cluster_information,
                        contig_cluster_dictionary,
                        local_unclustered_table,
                    ) = runDBSCANs(
                        subset_table,
                        dimensions,
                        contig_markers,
                    ...
                    for cluster in cluster_information:
                        ...
                        global_cluster_info[new_cluster_name] = cluster_information[
                            cluster
                        ]
                    ...
                    for contig in contig_cluster_dictionary:
                        new_cluster_name = (
                            ...
                            + str(round_counter)
                            + "_"
                            + str(contig_cluster_dictionary[contig])
                        )
                        table_indices = local_current_table[
                            local_current_table["contig"] == contig
                        ].index.tolist()
    ...
    for dimensions in [2, 3]:
        while True:
            ...
            for cluster in cluster_information:
                new_cluster_name = (
                    "DBSCAN" + "_round" + str(round_counter) + "_" + str(cluster)
                )
            ...
            for contig in contig_cluster_dictionary:
                ...
                table_indices = master_table[
                    master_table["contig"] == contig
                ].index.tolist()
                master_table.at[table_indices[0], "cluster"] = new_cluster_name
...
# Output table
master_table.to_csv(
    path_or_buf=output_table_path, sep="\t", index=False, quoting=csv.QUOTE_NONE
)
