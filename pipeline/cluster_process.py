...
parser = argparse.ArgumentParser(
    description="Script to summarize the assembly characteristics and taxonomy of binned clusters, as well producing individual cluster fasta files"
)
parser.add_argument(
    "-b",
    "--bin_table",
...
parser.add_argument(
    "-t",
    "--do_taxonomy",
    help="carry out taxonomic analysis on the clusters (you must have already run make_taxonomy_table.py)",
    action="store_true",
)
parser.add_argument(
    "-db", "--db_dir", metavar="<dir>", help="Path to directory with taxdump files"
)
...
if do_taxonomy:
    if not db_dir:
        print(
            "Error! If you want to analyze taxonomy, you need to specify a path to database files (--db_dir)"
        )
        exit(1)
...
contig_info = (
    dict()
)  # Will hold dictionaries for each contig, storing length, gc and cov (to calculate weighted av. of gc and cov later for each cluster)
...
summary_table.write(
    "cluster\tsize\tlongest_contig\tn50\tnumber_contigs\tcompleteness\tpurity\tav_cov\tav_gc\n"
)
...
if do_taxonomy:
    ...
    cluster_taxonomy_command = "cluster_taxonomy.py -t {} -c {} -x {} -o {}".format(
        bin_table_path, cluster_column_heading, db_dir, taxonomy_output_path
    )
