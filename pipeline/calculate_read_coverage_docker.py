...
parser = argparse.ArgumentParser(
    description="Script to tabulate paired-end or single read \
...
parser.add_argument(
    "-F",
    "--forward_reads",
    metavar="<reads.fastq|reads.fastq.gz>",
...
parser.add_argument(
    ...
    "--single_reads",
    metavar="<reads.fastq|reads.fastq.gz>",
    help="Single (*.fastq|*.fastq.gz) reads",
    nargs="*",
)
parser.add_argument(
    "-o", "--output_dir", metavar="<dir>", help="Path to output directory", default="."
)
...
if not os.path.isfile(assembly_file):
    print(
        ("Error! Could not find assembly file at the following path: " + assembly_file)
    )
    exit(1)
...
if not (forward_read_path_list or reverse_read_path_list or single_read_path_list):
    print(
        "Error! You need to specify some reads, with -F/-R (paired) and/or -S (single)"
    )
    exit(1)
...
if forward_read_path_list:
    for path in forward_read_path_list:
        ...
        docker_forward_read_path_list.append(
            read_directories[read_dir] + "/" + read_filename
        )
    for path in reverse_read_path_list:
        ...
        if read_dir not in read_directories:
            ...
            docker_dir_name = "/reads_dir_" + str(read_directory_counter)
            read_directories[read_dir] = docker_dir_name
        docker_reverse_read_path_list.append(
            read_directories[read_dir] + "/" + read_filename
        )
...
if single_read_path_list:
    for path in single_read_path_list:
        ...
        if read_dir not in read_directories:
            ...
            docker_dir_name = "/reads_dir_" + str(read_directory_counter)
            read_directories[read_dir] = docker_dir_name
        docker_single_read_path_list.append(
            read_directories[read_dir] + "/" + read_filename
        )
...
if docker_forward_read_path_list:
    calculate_read_coverage_command = (
        calculate_read_coverage_command
        + " --forward_reads "
        + " ".join(docker_forward_read_path_list)
    )
    calculate_read_coverage_command = (
        calculate_read_coverage_command
        + " --reverse_reads "
        + " ".join(docker_reverse_read_path_list)
    )
...
if docker_single_read_path_list:
    calculate_read_coverage_command = (
        calculate_read_coverage_command
        + " --single_reads "
        + " ".join(docker_single_read_path_list)
    )

# Construct the docker run command
docker_command = "docker run --volume {}:/output:rw --detach=false --rm".format(
    output_dir
)

# Now we need to add how every many read dirs there are
for host_path in read_directories:
    docker_command = docker_command + " --volume {}:{}:rw".format(
        host_path, read_directories[host_path]
    )
