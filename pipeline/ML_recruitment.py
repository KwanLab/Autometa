...
def normalizeKmers(count_matrix):  # list of lists, not a np matrix
    ...
    for i in range(0, len(list(unique_k_mers.keys()))):
        non_zero_counts = 0
    ...
    for i in range(0, len(count_matrix)):
        ...
        for j in range(0, len(list(unique_k_mers.keys()))):
...
def redundant_marker_prediction(
    contig_name, predicted_cluster, pandas_table, cluster_column_name
):
    ...
    cluster_df = pandas_table.loc[
        pandas_table[cluster_column_name] == predicted_cluster
    ]
    cluster_PFAMs = []
...
def calculateClusterStats(pandas_table, cluster_column, life_domain="bacteria"):
    ...
    if life_domain == "bacteria":
        ...
        print(
            (
                "Unexpected life domain: {}. Please select 'bacteria' or 'archaea'.".format(
                    life_domain
                )
            )
        )
    ...
    for count, contig in enumerate(pandas_table["contig"]):
        ...
        if num_markers >= 1:
            if label not in cluster_dict:
                cluster_dict[label] = {}
                cluster_dict[label]["PFAMs"] = pandas_table.iloc[count][
                    "single_copy_PFAMs"
                ].split(",")
            else:
                cluster_dict[label]["PFAMs"] += pandas_table.iloc[count][
...
if confidence_cutoff % bootstrap_iterations != 0 and len(
    str(int(confidence_cutoff))
) == len(str(bootstrap_iterations)):
...
for count, contig in enumerate(contig_table["contig"]):
    ...
    if use_taxonomy_info:
        ...
        taxonomy = (
            tax_phylum + tax_class + tax_order + tax_family + tax_genus + tax_species
        )
...
while num_markers_classifed > 0:
    ...
    num_unclustered_contigs = (
        contig_table[cluster_column_name].tolist().count(unclustered_name)
    )
    unclustered_contig_feature_list = []
    unclustered_contig_list = []
    print(
        (
            "Recruiting {} unclustered sequences with {} training contigs. This could take a while...".format(
                num_unclustered_contigs, len(features)
            )
        )
    )
    ...
    multiprocessed_output = Parallel(n_jobs=processors)(
        delayed(calculate_bootstrap_replicates)(features, bootstrap_iterations)
        for unclustered_features in unclustered_contig_feature_list
    )
    ...
    for count, output_tuple in enumerate(multiprocessed_output):
        ...
        redundant, is_marker_contig = redundant_marker_prediction(
            contig, ML_prediction, temp_contig_table, cluster_column_name
        )
        global_contig_index = contig_index_dict[contig]
        if confidence >= confidence_cutoff and not redundant:
            print(
                (
                    "ML predictions and jackknife confidence for contig {}: {},{}".format(
                        contig, ML_prediction, confidence
                    )
                )
            )
    ...
    cluster_stats_dict = calculateClusterStats(
        contig_table, cluster_column_name, kingdom
    )
    completeness_list = []
    purity_list = []
    for cluster, info_dictionary in list(cluster_stats_dict.items()):
    ...
    elapsed_time = time.strftime(
        "%H:%M:%S", time.gmtime(round((time.time() - iteration_start_time), 2))
    )
    print(
        (
            "{} ({} marker contigs) of {} predictions ({} bp) were {}% confident and non-redundant for iteration {} in {} (HH:MM:SS). Mean completeness,purity: {},{}".format(
                num_confident_predictions,
                num_markers_classifed,
                num_predictions,
                recruited_sequence_length,
                confidence_cutoff,
                iteration,
                elapsed_time,
                mean_completeness,
